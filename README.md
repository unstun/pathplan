...
- Minimal dependencies: NumPy-only core, no ROS; RL guidance runs with or without PyTorch (deterministic NumPy fallback).
- Ready-to-run demo scenarios plus clear stats (path length, expansions, timing, node counts) for quick baselining.
- Modular library: small, readable modules for motion primitives, heuristics, collision checking, RL guidance, and planners.
- Extensible hooks: swap heuristics, change motion primitives, plug in real RL models, or tighten safety via map inflation.
...
- **RL-guided multi-heuristic Hybrid A\***: Two synchronized queues (anchor + RL) maintain completeness while biasing expansions with a learned value estimate and action-ranking policy. Optional `rl_top_k` reduces branching for speed.
...
- **Maps**: occupancy grid (`GridMap`) with world/grid conversions, inflation, random free-state sampling, and robot-aligned occupancy patches for RL features.
...
- `pathplan/map_utils.py` - occupancy grid, inflation, sampling, local patches for RL.
...
- `pathplan/rl_guided_hybrid.py` - multi-queue Hybrid A* with RL guidance for heuristics and action ordering.
- `pathplan/rl_models.py` - lightweight RL-like value/policy using local occupancy patches (torch-optional stubs).
...
- **RL-guided Hybrid A\*** (`RLGuidedHybridPlanner`):
  - Two queues: anchor uses admissible heuristic; RL queue uses learned value + policy-ranked actions.
  - RL features from a local occupancy patch (robot frame), goal distance/heading, and occupancy statistics.
  - Configurable `rl_top_k` to limit branching; `rl_weight` to scale the learned heuristic.
...
Swap in `RLGuidedHybridPlanner` for faster searches, or `RRTStarPlanner` for kinodynamic sampling-based planning.
...
- **RL guidance**: drop in real models inside `rl_models.RLGuidance` (value/policy) or adjust patch size/features.
...
- **RL-guided Hybrid**: adds `expansions_anchor`, `expansions_rl`, `expansions_total`.
...
- RL-friendly: guidance hooks are present but optional; runs identically with just NumPy.
